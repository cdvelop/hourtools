...
func TestDayOfYear(t *testing.T) {
	tests := []struct {
		...
		{2023, 2, 29, -1, "Fecha no válida"}, // Fecha inválida en año no bisiesto
		{2024, 4, 31, -1, "Fecha no válida"}, // Fecha inválida en año bisiesto
	}

	for _, test := range tests {
		dayOfYear, err := timetools.DayOfYear(test.year, test.month, test.day)
		if dayOfYear != test.expectedDayOfYear || (err != nil && err.Error() != test.expectedError) {
